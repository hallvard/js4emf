...
public class EditJavascriptCommandHandler extends AbstractHandler {
	...
	private EObject getScriptedSelection(ISelection selection) {
		if (selection instanceof IStructuredSelection && ((IStructuredSelection)selection).getFirstElement() instanceof EObject) {
			...
			FeatureValueProvider<String> scriptSourceFeatureValueProvider = JavascriptSupportFactory.getInstance().getJavascriptSupport(eObject).getScriptSourceFeatureValueProvider(eObject);
	...
	private void openEditorForScriptedEObject(final EObject eObject, final IEditingDomainProvider editingDomainProvider) {
		if (eObject != null) {
			FeatureValueProvider<String> scriptSourceFeatureValueProvider = ScriptSourceView.getScriptSourceFeatureValueProvider(eObject);
			if (scriptSourceFeatureValueProvider != null) {
				String scriptSource = scriptSourceFeatureValueProvider.getFeatureValue(eObject);
				if (scriptSource == null) {
					scriptSource = "";
				}
				Resource resource = eObject.eResource();
				if (resource != null) {
					final String fragment = JavascriptSupport.SCRIPT_SOURCE_FEATURE_NAME + convertFragmentToTempName(resource.getURIFragment(eObject));
